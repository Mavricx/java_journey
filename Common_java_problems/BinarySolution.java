...
public class BinarySolution {
    public static String addBinary(String a, String b) {
        ...
        while (p >= 0 || q >= 0) {
            ...
            if (p >= 0)
                sum = sum + a.charAt(p--) - '0'; // adding number each individual number
            if (q >= 0)
                sum = sum + b.charAt(q--) - '0';
            carry = sum > 1 ? 1 : 0; // carry will be 1 is sum is 2 or 3
            ans.append(sum % 2); // changing sum so that only 0 or 1 will be selected
        }
        if (carry != 0)
            ans.append(carry); // after end of loop if carry is not 0 then add it at front
